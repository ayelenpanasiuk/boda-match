      ...
      <div className="min-h-full flex flex-col px-4 py-6">
        ...
        <div className="flex items-center justify-between mb-6 px-1">
          <button
            ...
            className="w-10 h-10 flex items-center justify-center rounded-full border border-white/10 active:scale-90 transition-transform"
        ...
        <div
          className="flex rounded-2xl p-1 mb-6 border border-white/10"
        ...
        >
          {[['matches','matches',<Heart size={15}/>],['favs','favoritos',<Bookmark size={15}/>]].map(([v,l,icon]) => (
            <button
              key={v}
              onClick={() => setTab(v)}
              className={`flex-1 py-2.5 rounded-xl font-body text-sm font-medium transition-all flex items-center justify-center gap-1.5 ${
                tab === v ? 'text-white' : 'text-white/40'
              }`}
        ...
        {list.length === 0 ? (
          <div className="flex flex-col items-center justify-center flex-1 text-center py-16">
            ...
            <p className="font-display text-2xl text-white mb-2">
              {isMatchTab ? 'Todavía no tenés matches' : 'Todavía no tenés favoritos'}
            </p>

            <p className="font-body text-white/40 text-sm mb-6">
              {isMatchTab
                ? 'Seguí jugando, el match puede aparecer en cualquier momento 😏'
                : 'Guardá perfiles con el marcador en las cartas'}
            </p>

            <button className="btn-wine max-w-[200px]" onClick={() => navigate('/feed')}>
              Seguir viendo
            </button>
          </div>
        ) : (
              ...
              >
                <div className="relative h-44">
                  ...
                  <div
                    ...
                    style={{ background:'linear-gradient(to top,rgba(0,0,0,.8) 0%,rgba(0,0,0,0) 55%)' }}
                  />
                  ...
                  {isMatchTab && (
                    <div
                      ...
                      style={{ background:'rgba(107,21,48,.75)', backdropFilter:'blur(6px)' }}
                    >
                      <span className="font-body text-white text-[10px]">🔥 ¡HAY MATCH!</span>
                    </div>
                  )}
                </div>
                ...
                {open === p.id && (
                  <div
                    className="p-3 space-y-2 animate-slideUp"
                    style={{ background:'rgba(255,255,255,.04)' }}
                  >
                    {isMatchTab ? (
                      <>
                        <p className="font-body text-white/55 text-xs">
                          Este match puede ser el comienzo de algo...
                        </p>

                        <p className="font-body text-white/55 text-xs">
                          o de un gran momento en la fiesta 💃
                        </p>
                      ...
                      <div
                        className="flex items-center gap-2 rounded-xl p-2.5 border border-white/10"
                        style={{ background:'rgba(255,255,255,.06)' }}
                      >
                        <Lock size={14} className="text-white/30 shrink-0" />
                        <span className="font-body text-white/40 text-xs">
                          Contacto visible al hacer match
                        </span>
                      </div>
                    )}
                  </div>
